...
%     title("\fontsize{14} Target Image");
    ...
    %Evaluating metrics
    max_angle = printEvaluations(thetas, metric_vals, local_max, A, B, correct_angle);
    
    final_img = buildFan(original_ref, max_angle);
    final_img = imgaussfilt(final_img,sd) * A + B;
    
    figure(2)
%     xlabel('x');
%     ylabel('y');set(get(gca,'YLabel'),'Rotation',0);
%     title("\fontsize{14} Final Angle = " + max_angle + " Radians");
